...
1. Git is a system that allows you to track changes made to programs and code. Github is a cloud based hosting service that allows you to manage Git repositories. 
2. We use branches to safely edit and make changes in the code without risking making changes to the orginal code.
3. Pull requests let other in our team know about the changes that we have made to the code. 
4. 'git checkout branch name' is one way that you can switch between branches. 
5. 'git fetch' checks the original file for changes in the repository allowing you to see what your team has worked on or changed but doesn't copy the files. 'git pull' checks for changes and then copies them into your repository. 'git merge' merges all branches into one singular branch.
6. Merge conflicts are when there is competing code on the same lines that don't allow all of the repository to come together. You can fix this using a text editor and the command line. 